...
@section('meta_title',__('Edit Board Of Directory'))

@section('page_name',__('Edit Board Of Directory'))

@section('page_description',__('Edit Board Of Directory'))
@section('name')
    <li class="breadcrumb-item">
        <a href="{{ route('dashboard') }}"> <i class="feather icon-home"></i> </a>
    </li>
    <li class="breadcrumb-item"><a href="#!">{{ __('Edit Board Of Directory') }}</a>
    </li>
@endsection
@section('content')
<div class="row">
    <div class="col-sm-12">
        ...
        <div class="card">
            ...
            <div class="card-block">
                <form action="{{ route('boardofdirectories.update',encrypt($board->id)) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="form-group row">
                        <label class="col-sm-2 col-form-label">{{ __(' Designation Title') }}</label>
                        <div class="col-sm-10">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <input type="text" name="name" id="name" value="{{ old('name') ?? $board->name}}" placeholder="{{ __('Enter Name') }}" class="form-control @error('name') form-control-danger @enderror">
                           <span class="messages">
                                @error('name')
                                    <p class="text-danger error">{{ $message }}</p>
                                @else
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <input type="text" name="designation" id="designation" value="{{ old('designation') ?? $board->designation }}" placeholder="{{ __('Enter Designation') }}" class="form-control @error('designation') form-control-danger @enderror">
                            @error('designation')
                                <p class="text-danger error">{{ $message }}</p>
                            @else
                                <p class="text-muted">{{ __('') }}</p>
