...
@section('meta_title', __(' Edit Placement Service'))

@section('page_name', __(' Edit Placement Service'))

@section('page_description', __(' EditPlacement Service'))

@section('name')
    <li class="breadcrumb-item">
        <a href="{{ route('dashboard') }}"> <i class="feather icon-home"></i> </a>
    </li>
    <li class="breadcrumb-item"><a href="#!">{{ __(' Edit Placement Service') }}</a></li>
@endsection
    ...
    <div class="col-sm-12">
        ...
        <div class="card">
            ...
            <div class="card-block">
                <form action="{{ route('placement.update', encrypt($placements->id)) }}" method="POST">
                    @csrf
                    @method('PUT')
                    {{-- <div class="form-group row">
                        <label class="col-sm-2 col-form-label">{{ __('Title') }}</label>
                        <div class="col-sm-10">
                            <input type="text" name="title" id="title" value="{{ old('title') ?? $placements->title }}" placeholder="{{ __('Enter Title') }}" class="form-control @error('title') form-control-danger @enderror">
                            <span class="messages">
                                @error('title')
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <textarea name="brief_description" id="brief_description" rows="8" placeholder="{{ __('Enter Brief Description') }}" class="form-control @error('brief_description') form-control-danger @enderror">{{ old('brief_description') ?? $placements->brief_description }}</textarea>
